...
for idx, k in enumerate(np.unique([all_labels,all_preds])):
...
all_txtlabels =[]
all_txtpreds =[]
for el in list( all_labels):
    all_txtlabels.append(class2txt[el] )
for el in list( all_preds):
    all_txtpreds.append(class2txt[el] )



 # classes absent from my model
others = ['Avalanche_and_rockfall_protection_structures', 'Dumps', 'Energy_supply_plants',
        'Horticulture', 'Industrial_and_commercial_areas_<_1_ha', 'Public_parks', 'Quarries_mines', 
        'Railway_surfaces']
additional= [102,124,141,144,145,161,203,422]

# extract all classes name in a list
classes =list(np.unique([all_txtlabels,all_txtpreds]))
...
#df_cm.to_csv(r'/home/valerie/Python/landuse/Images/confusion_matrix/cm_FHWN_ADELE.csv',index = False)
x = plt.figure()
...
ax.set(title="Heatmap FHNW" ,
    xlabel="Predicted Label",
    ylabel="True Label")

fn = '/home/valerie/Python/landuse/Images/confusion_matrix/cm_FHNW_adele_2801.png'
...
others = ['Avalanche and rockfall protection structures', 'Dumps', 'Energy supply plants',
        'Horticulture', 'Industrial and commercial areas < 1 ha', 'Public parks', 'Quarries mines', 
        'Railway surfaces'] # classes absent from my model
...
#----------------------------------
if True :
    file_name = '/home/valerie/Python/landuse/Images/testing/results_FHNW.txt'
    fd = open(file_name,'w')
